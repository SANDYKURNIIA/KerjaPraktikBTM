...
class M_Jurnal_pendapatan extends CI_Model
{
    ...
    public function SelectLaporanRangeJurnalRajal($first_date, $second_date)
    {
        ...
        and (DATE(d.tgl_input) BETWEEN '$first_date' and '$second_date') and a.status = 0 and a.kode_akun !='704.02.421'
        and d.status = 1
        group by a.id_akun
        ) as b
        ")->result();
    ...
    public function SelectLaporanJurnalRajal()
    {
        ...
        FROM akun_non_pelayanan a, pendapatan_kasir d
        WHERE a.id_pelayanan=d.id_pelayanan  and d.keterangan != 'asuransi' 
        and (DATE(d.tgl_input) = '$tgl') and a.status = 0 and a.kode_akun !='704.02.421'
        group by a.id_akun
        ) as b
        ")->result();
    }
    ...
    public function selectPendapatanKasir($mulai, $akhir) //RAJAL deposite untuk set jurnal
    {
        ...
        if ($mulai != '' && $akhir != '') {
            ...
            SELECT (d.tgl_input) tgl_input, ps.nama pasien, ps.no_rm, (d.total_bayar) total, s.nama staff,d.keterangan,d.id_pendapatan,d.id_pelayanan
            ...
            and (DATE(d.tgl_input) BETWEEN '$mulai' and '$akhir')  and p.status=1 
            ...
            -- and (d.tgl_pulang is null or date(d.tgl_input) != date(d.tgl_pulang))
            group by d.id_pendapatan

            UNION ALL
            SELECT (d.tgl_input) tgl_input, ps.nama pasien, ps.no_rm, (d.selisih) total, s.nama staff,d.keterangan,d.id_pendapatan,d.id_pelayanan
            FROM pelayanan p, staff s,pendapatan_kasir d,pasien ps
            WHERE p.id_pasien=ps.no_rm and s.id_staff=d.id_staff and d.id_pelayanan=p.id_pelayanan 
            and (DATE(d.tgl_input) BETWEEN '$mulai' and '$akhir')  and p.status=1
            and p.cara_bayar != '42' and p.id_pelayanan not in(select id_pelayanan from history_pelayanan_ranap where status = 1) 
        ...
        ")->result();
            ...
            $hasil = $this->db->query("SELECT '403.01.000' kode_akun,g.total total_akun,g.keterangan,IFNULL(b.nama_bank,'') as bank,g.id_pelayanan, g.pasien from (
                SELECT (d.tgl_input) tgl_input, ps.nama pasien, ps.no_rm, (d.total_bayar) total, s.nama staff,d.keterangan,d.id_pendapatan,d.id_pelayanan
                FROM pelayanan p, pasien ps, staff s,pendapatan_kasir d
                WHERE p.id_pasien=ps.no_rm and s.id_staff=d.id_staff and d.id_pelayanan=p.id_pelayanan 
                and d.tgl_input like '%$tgl%' and p.status=1 
                -- and (DATE(p.tgl_keluar) not BETWEEN '$tgl' and '$tgl')
                and p.cara_bayar = '42' and p.id_pelayanan not in(select id_pelayanan from history_pelayanan_ranap ) and d.total_bayar != 0 and d.status=1 and d.no_jurnal_deposite is null
                -- and (d.tgl_pulang is null or date(d.tgl_input) != date(d.tgl_pulang))
                UNION ALL
            SELECT (d.tgl_input) tgl_input, ps.nama pasien, ps.no_rm, (d.selisih) total, s.nama staff,d.keterangan,d.id_pendapatan,d.id_pelayanan
            FROM pelayanan p, staff s,pendapatan_kasir d,pasien ps
            WHERE p.id_pasien=ps.no_rm and s.id_staff=d.id_staff and d.id_pelayanan=p.id_pelayanan 
            and (DATE(d.tgl_input) = '$tgl')  and p.status=1
            and p.cara_bayar != '42' and p.id_pelayanan not in(select id_pelayanan from history_pelayanan_ranap where status = 1) 
            and d.selisih != 0 and d.status=1 and d.status_jurnal =0
            and d.tipe ='SELISIH'
                ) as g
    ...
    public function Set_jurnal_rajal()
    {
        ...
        FROM (SELECT a.kode_akun,sum(a.total_akun) total_akun , a.jenis_akun
        FROM akun_non_pelayanan a, pendapatan_kasir d
        WHERE  a.id_pelayanan = d.id_pelayanan and d.keterangan !='asuransi' and a.status = 0 and d.status=1
        and (DATE(d.tgl_input) = '$tgl') and a.kode_akun !='704.02.421'
        GROUP BY a.kode_akun
    ...
    public function Set_jurnal_rajal_range($first_date, $second_date)
    {
        ...
        FROM akun_non_pelayanan a, pendapatan_kasir d
        WHERE  a.id_pelayanan = d.id_pelayanan and d.keterangan !='asuransi' and a.status = 0 and d.status=1
        and (DATE(d.tgl_input) BETWEEN '$first_date' and '$second_date') and a.kode_akun !='704.02.421'
        GROUP BY a.kode_akun
    ...
    public function SelectLaporanJurnalRanap()
    {
        ...
        and d.tgl_input like '%$tgl%' and a.status = 0
        ")->result();
    }
    ...
    public function selectPendapatanKasirRanap($mulai, $akhir) //RANAP
    {
        ...
        ")->result();
            ...
            $hasil = $this->db->query("SELECT '403.01.000' kode_akun,sum(total_akun) total_akun, keterangan, bank,id_pelayanan,pasien FROM(
                SELECT g.total total_akun, g.keterangan,IFNULL(b.nama_bank,' ') as bank,g.id_pelayanan,g.pasien from (
                SELECT (d.tgl_input) tgl_input, ps.nama pasien, ps.no_rm, (d.total_bayar) total, s.nama staff,d.keterangan,d.id_pendapatan,d.id_pelayanan
                FROM pelayanan p, staff s,pendapatan_kasir d,pasien ps
                WHERE p.id_pasien=ps.no_rm and s.id_staff=d.id_staff and d.id_pelayanan=p.id_pelayanan 
                and (DATE(d.tgl_input) = '$tgl')  and p.status=1 and p.status_rawat != 'selesai'
                and p.cara_bayar = '42' and p.id_pelayanan in(select id_pelayanan from history_pelayanan_ranap where status = 1) 
                and d.total_bayar != 0 and d.status=1 and d.no_jurnal_deposite is null
                -- and (d.tgl_pulang is null or date(d.tgl_input) != date(d.tgl_pulang))
