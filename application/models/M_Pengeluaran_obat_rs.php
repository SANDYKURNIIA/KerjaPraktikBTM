...
class M_Pengeluaran_obat_rs extends CI_model
{
    ...
    public function selectNoDokumenRetur()
    {
        ...
        $tgl = date("Y-m");
        $this->db->select('MAX(index_dok) max');
        $this->db->from('pengeluaran_obat_rs p');
        $this->db->like('tgl_input', $tgl);
        return $this->db->get()->result();
    }
    ...
    public function selectLaporanRetur($mulai, $akhir)
    {
        ...
        $this->db->from('stok_logistik s, list_logistik l,detail_retur_obat d,pengeluaran_obat_rs p');
