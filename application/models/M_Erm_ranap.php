...
class M_Erm_ranap extends CI_Model
{
    ...
    public function update_asesmen_2_tabel($id_form, $data_utama, $data_tambahan)
    {
        $this->db->trans_start();

        // Update Tabel Utama (form_ass_per_ranap)
        $this->db->where('id_form', $id_form);
        $this->db->update('form_ass_per_ranap', $data_utama);

        // Update Tabel Kedua (form_ass_per_ranap2)
        $this->db->where('id_form', $id_form);
        $this->db->update('form_ass_per_ranap2', $data_tambahan);
    ...
    public function cetakAssPerRanap($id)
    {
        $this->db->select('
            f.*, 
            f2.kom_bicara, f2.kom_serangan_bicara_kapan, f2.kom_bahasa, f2.kom_bahasa_daerah_detail, 
            f2.kom_bahasa_lain_detail, f2.kom_perlu_penterjemah, f2.kom_penterjemah_bahasa, 
            f2.kom_bahan_isyarat, f2.kom_hambatan_belajar, f2.kom_cara_belajar_disukai, 
            f2.kom_tingkat_pendidikan, f2.kom_potensial_kebutuhan_pembelajaran, f2.kom_potensial_lain_detail, 
            f2.kognitif_izin_informasi, f2.paed_lama_kehamilan, f2.paed_komplikasi_tipe, 
            f2.paed_masalah_maternal, f2.paed_persalina_tipe, f2.paed_penyulit_persalinan, 
            f2.paed_post_natal, f2.paed_riwayat_imunisasi, f2.paed_keluhan_tumbuh_kembang, 
            f2.paed_diagnosa_medis, f2.paed_lahir_umur_kehamilan, f2.paed_pernah_dirawat, 
            f2.paed_pernah_dirawat_dimana, f2.paed_pernah_dirawat_kapan, f2.paed_lk_saat_lahir, 
            f2.paed_bb_saat_lahir, f2.paed_tb_saat_lahir, f2.paed_asi_sampai_umur, 
            f2.paed_susu_formula_mulai, f2.paed_makanan_tambahan_umur, f2.paed_masalah_neonatus, 
            f2.paed_tengkurap_bln, f2.paed_duduk_bln, f2.paed_merangkak_bln, 
            f2.paed_berdiri_bln, f2.paed_berjalan_bln,
            p.nama, p.tgl_lahir, p.jenis_kelamin, p.agama,
            b.tgl_masuk, c.nama cara_bayar
        ');
        $this->db->from('form_ass_per_ranap f');
        $this->db->join('form_ass_per_ranap2 f2', 'f.id_form = f2.id_form', 'left');
        $this->db->join('pelayanan b', 'f.id_pelayanan = b.id_pelayanan', 'left');
        $this->db->join('pasien p', 'f.no_rm = p.no_rm', 'left');
        $this->db->join('cara_bayar c', 'b.cara_bayar = c.id_cara_bayar', 'left');
    ...
    public function checkAsesmenPerawatRanap($id_pelayanan, $id_history = null)
    {
        $this->db->from('form_ass_per_ranap_2');
        $this->db->where('id_pelayanan', $id_pelayanan);
        if (!empty($id_history)) {
            $this->db->or_where('id_history', $id_history);
        }
        $q2 = $this->db->get();
        if ($q2 && $q2->num_rows() > 0) {
            return true;
        ...
        $q1 = $this->db->get();
        return ($q1 && $q1->num_rows() > 0);
    }

    public function getAsesmenPerawatRanap($id_pelayanan, $id_history = null)
    {
        $this->db->select('f.*, f2.id_form_2, f2.id_staf AS id_staf_2, f2.kom_bicara, f2.kom_serangan_bicara_kapan, f2.kom_bahasa, f2.kom_bahasa_daerah_detail, f2.kom_bahasa_lain_detail, f2.kom_perlu_penterjemah, f2.kom_penterjemah_bahasa, f2.kom_bahan_isyarat, f2.kom_hambatan_belajar, f2.kom_cara_belajar_disukai, f2.kom_tingkat_pendidikan, f2.kom_potensial_kebutuhan_pembelajaran, f2.kom_potensial_lain_detail, f2.kognitif_izin_informasi, f2.paed_lama_kehamilan, f2.paed_komplikasi_tipe, f2.paed_masalah_maternal, f2.paed_persalina_tipe, f2.paed_penyulit_persalinan, f2.paed_post_natal, f2.paed_riwayat_imunisasi, f2.paed_keluhan_tumbuh_kembang, f2.paed_diagnosa_medis, f2.paed_lahir_umur_kehamilan, f2.paed_pernah_dirawat, f2.paed_pernah_dirawat_dimana, f2.paed_pernah_dirawat_kapan, f2.paed_lk_saat_lahir, f2.paed_bb_saat_lahir, f2.paed_tb_saat_lahir, f2.paed_asi_sampai_umur, f2.paed_susu_formula_mulai, f2.paed_makanan_tambahan_umur, f2.paed_masalah_neonatus, f2.paed_tengkurap_bln, f2.paed_duduk_bln, f2.paed_merangkak_bln, f2.paed_berdiri_bln, f2.paed_berjalan_bln, f2.created_at AS created_at_2, f2.updated_at AS updated_at_2');
        $this->db->from('form_ass_per_ranap f');
        $this->db->join('form_ass_per_ranap_2 f2', 'f2.id_form = f.id_form', 'left');
        $this->db->where('f.id_pelayanan', $id_pelayanan);
        $this->db->order_by('f.id_form', 'DESC');
        $this->db->limit(1);
        $query = $this->db->get();
        ...
        $this->db->from('form_ass_per_ranap_2');
        $this->db->where('id_pelayanan', $id_pelayanan);
        $this->db->order_by('id_form_2', 'DESC');
        $this->db->limit(1);
        $query2 = $this->db->get();
