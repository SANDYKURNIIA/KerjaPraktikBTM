...
class M_Erm_ranap extends CI_Model
{
    ...
    {
        $this->db->select('e.*, s.nama AS nama_staff, s.qr_code AS ttd_staff, d.nama AS nama_dokter, d.foto AS ttd_dokter');
        $this->db->from('ews_maternity e');
        $this->db->join('staff s', 'e.id_staff = s.id_staff', 'left');
        $this->db->join('history_pelayanan_ranap h', 'e.id_history = h.id_history', 'left');
        $this->db->join('dokter d', 'h.dpjp = d.id_dokter', 'left');
        $this->db->where('e.id_pelayanan', $id_pelayanan);
        $this->db->where('e.id_history', $id_history);
        $this->db->order_by('e.id_form', 'DESC');
        return $this->db->get()->row();
    }

    public function get_ews_maternity_by_ids($id_array)
    {
        $this->db->select('e.*, s.nama AS nama_staff, s.qr_code AS ttd_staff, d.nama AS nama_dokter, d.foto AS ttd_dokter');
        $this->db->from('ews_maternity e');
        $this->db->join('staff s', 'e.id_staff = s.id_staff', 'left');
        $this->db->join('history_pelayanan_ranap h', 'e.id_history = h.id_history', 'left');
        $this->db->join('dokter d', 'h.dpjp = d.id_dokter', 'left');
        $this->db->where_in('e.id_form', $id_array);
        return $this->db->get()->result();
    }

    public function selectPemantauanEwsMaternitySehari($id_pelayanan, $id_history = null)
    {
        $this->db->select('e.*, s.nama AS nama_staff, r.tipe AS nama_ruangan');
        $this->db->from('ews_maternity e');
        $this->db->join('staff s', 'e.id_staff = s.id_staff', 'left');

        // Ganti join riwayat_kamar dengan history_pelayanan_ranap berdasarkan id_history
        $this->db->join('history_pelayanan_ranap h', 'e.id_history = h.id_history', 'left');
        $this->db->join('ruangan r', 'h.id_kamar = r.id_ruangan', 'left');
        ...
        $this->db->group_by('e.id_form'); // Mencegah duplikasi data jika ada join yang berlebih
        $this->db->order_by('e.tanggal', 'DESC');
        return $this->db->get()->result();
    }

    public function getNamaRuanganByPelayanan($id_pelayanan)
    {
        $this->db->select('r.nama_ruangan');
        $this->db->from('history_pelayanan_ranap h');
        $this->db->join('ruangan r', 'r.id_ruangan = h.id_kamar', 'left');
        $this->db->where('h.id_pelayanan', $id_pelayanan);
        $query = $this->db->get()->row();

        return $query ? $query->nama_ruangan : '-';
    }
    public function edit_ews_maternity_data($id_form, $data)
    {
        $this->db->where('id_form', $id_form);
        return $this->db->update('ews_maternity', $data);
    }
}
