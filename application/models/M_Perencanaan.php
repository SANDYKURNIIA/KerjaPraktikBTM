...
class M_Perencanaan extends CI_model
{
    ...
    public function selectNoDokumenRetur()
    {
        ...
        $tgl = date("Y-m");
        $this->db->select('MAX(index_dok) max');
        $this->db->from('faktur_retur_obat');
        $this->db->like('tgl_input', $tgl);
        return $this->db->get()->result();
    }
    ...
    {
        $this->db->select('l.nama,l.produsen,l.satuan_terbesar,l.diskon, f.harga, f.status, f.jumlah,f.total,f.frek, f.id_detail, f.status,f.id_detail_usulan');
    ...
    {
        $this->db->select('l.id_logistik,l.nama,l.produsen,l.satuan_terbesar,l.diskon, f.harga, f.status, f.jumlah,f.total,f.frek, f.id_detail, f.status');
        $this->db->from('list_logistik l, detail_usulan_logfar f');
        $this->db->where('f.id_list=l.id_logistik');
        $this->db->where('f.id_faktur', $idFaktur);
        $this->db->order_by('l.produsen, l.nama asc');
    ...
    public function selectDataRetur()
    {
        ...
        $this->db->distinct();
        $this->db->select('r.*');
        $this->db->from('faktur_retur_obat r');
        $this->db->like('r.tgl_input', $tgl);
        $this->db->order_by('tgl_input desc');
        return $this->db->get()->result();
    }
    public function selectRangeRetur($mulai, $akhir)
    {
        $this->db->distinct();
        $this->db->select('f.* ');
        $this->db->from('faktur_retur_obat f');
        $this->db->where('f.tgl_input >=', $mulai);
        $this->db->where('f.tgl_input <=', $akhir);
        $this->db->order_by('f.tgl_input');
        return $this->db->get()->result();
    }
    public function getListRetur($idFaktur)
    {
        $this->db->select('l.nama, l.produsen,l.satuan_terkecil, l.satuan_terbesar, f.*');
        $this->db->from('list_logistik l, detail_retur_obat f');
        $this->db->where('f.id_list=l.id_logistik');
        $this->db->where('f.id_faktur', $idFaktur);
        return $this->db->get()->result();
    }
    ...
    public function selectLaporanRetur($mulai, $akhir)
    {
        ...
        $this->db->select('l.*,s.frek jml_terima, s.tgl tgl_res, s.keterangan ket,f.no_dokumen,f.id_vendor');
        $this->db->from('stok_logistik s, list_logistik l,detail_retur_obat d,faktur_retur_obat f');
