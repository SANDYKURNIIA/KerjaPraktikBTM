...
class M_AllPoli extends CI_Model
{
    ...
    public function selectPasienRajal1($poli)
    {
        ...
        if ($poli == 'OBGYNE') {
            ...
            FROM v_pasien_rajal_kasir v
            where  (v.jenis_pelayanan = 'POLI' OR v.jenis_pelayanan = 'POLI PRIORITAS')
            and v.poli='OBGYNE' and v.id_pelayanan not in (select id_pelayanan from history_pelayanan_ranap where status = 1)");
            return $query->result();
        }
        if ($poli == 'THT') {
            $query = $this->db->query("SELECT v.*
            FROM v_pasien_rajal_kasir v
            where  (v.jenis_pelayanan = 'POLI' OR v.jenis_pelayanan = 'POLI PRIORITAS')
            and v.poli='THT' and v.id_pelayanan not in (select id_pelayanan from history_pelayanan_ranap where status = 1)");
            return $query->result();
        }
        if ($poli == 'MATA') {
            $query = $this->db->query("SELECT v.*
        ...
        if ($poli == 'CONTROL REHABILITAS MEDIC') {
            ...
            FROM v_pasien_rajal_kasir v
            where  (v.jenis_pelayanan = 'POLI' OR v.jenis_pelayanan = 'POLI PRIORITAS')
            and v.poli='KONTROL FISIO' and v.id_pelayanan not in (select id_pelayanan from history_pelayanan_ranap where status = 1)");
            return $query->result();
        }
        if ($poli == 'LABOR') {
            $query = $this->db->query("SELECT v.*
