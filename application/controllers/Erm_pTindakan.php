...
class Erm_pTindakan extends CI_Controller{
	...
	private function _where(){
		return "upload_penolakan_tindakan";
	}
	...
	public function form($id_pelayanan, $id_history){
		...
		$page_data['page_content'] = 'erm_form/view_Erm_pTindakan';
	...
	public function tampil_data(){
		...
		for($i = 0;$i < count($sql);$i++){
			$no = $i + 1;
			$tombol = "<button class='btn btn-success btn-icon-anim btn square'  onclick='pilih(\"" . $sql[$i]->id_pTindakan . "\")' '><i class='icon-rocket'></i></button>";
			$hapus = "<button class='btn btn-danger btn-icon-anim btn square' id='myHapus_".$sql[$i]->id_pTindakan."' onclick='hapus(\"" . $sql[$i]->id_pTindakan . "\")' '><i class='icon-trash'></i></button>";
