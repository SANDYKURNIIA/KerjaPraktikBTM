...
class Permintaan_obat_unit extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $tipe = $data->tipe;
        $page_data['page_content'] = 'page_content/Permintaan_obat_unit';
        // $page_data['obat'] = $this->M_Permintaan_obat->getNamaObatUnit();
    ...
    public function Tampil_permintaan_obat()
    {
        ...
        for ($i = 0; $i < count($page_data); $i++) {
            ...
            $out[$i] = array($no, $button, $button2, $tgl, $waktu, $nama);
        }
    ...
    public function insertFormPermintaanObatBaru()
    {
        ...
        if ($data_staff->tipe == "ok" || $data_staff->tipe == "rawatinap" || $data_staff->tipe == "igd" || $data_staff->tipe == "igdponek" || $data_staff->tipe == "cssd" || $data_staff->tipe == "polifisio" || $data_staff->tipe == "icu" || $data_staff->tipe == "nicu" || $data_staff->tipe == "polihemodialisa" || $data_staff->tipe == "labor" || $data_staff->tipe == "kemoterapi" || $data_staff->tipe == "korsek" || $data_staff->tipe == "casemanager") {
            $tipe = "depo ranap";
        } else if ($data_staff->tipe == "mcu") {
    ...
    public function tampil_list_tindakan()
    {
        ...
        for ($i = 0; $i < count($page_data); $i++) {
            ...
            $out[$i] = array($no, $nama, $produsen, $jml_req, $jml_terima, $satuan_terkecil, $status, $keterangan, $hapus);
        }
        if ($out == null) {
            echo '{"data":""}';
            exit;
    ...
    {
        $id_req = $this->input->post('id_req');
        $db = $this->db->get_where('detail_request', array('id_req' => $id_req))->row_array();
        if ($db['status' == 'DITERIMA']) {
            $out['status'] = "Tidak Dapat Dihapus karena permintaannya sudah diterima";
        } else {
    ...
    {
        $id_req = $this->input->post('id_req');
        $db = $this->db->get_where('detail_request', array('id_form' => $id_req, 'status' => 'DITERIMA'))->result_array();
        if (empty($db)) {
    ...
    public function getNamaObat()
    {
        ...
        $data = $this->M_Permintaan_obat->getNamaObatUnit($depo);
