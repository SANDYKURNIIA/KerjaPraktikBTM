...
class Bundle_encounter extends CI_Controller
{
    ...
    public function send()
    {
        ...
        foreach ($db_pasien as $row) {
            $this->create_encounter($row->id_pelayanan, $row->id_history);
        }
    }

    public function send_by_tgl($tgl)
    {
        $db_pasien = $this->db->query("SELECT f.id_pelayanan, f.id_history, p.no_ktp, p.nama, h.dpjp, b.tgl_keluar,f.tanggal, d.nama, d.nik
        from form_assesmen_awal_rajal f , pasien p, pelayanan b, history_pelayanan h,dokter d
        where p.no_rm=b.id_pasien and b.id_pelayanan =f.id_pelayanan and h.dpjp = d.id_dokter
        and p.no_ktp !='' and length(no_ktp)=16 and h.id_history =f.id_history and length(d.nik)=16
    ...
    public function create_encounter_bundle($id_pelayanan,$id_history)
    {
        ...
        if (!empty($db_erm)) {
            ...
            if (empty($satusehat_encounter)) {
                ...
                if (isset($id_patient) && isset($id_nakes)) {
                    ...
                    $kode_diagnosa = $this->formatAngka($db_diagnosa->kode);
    ...
    public function create_encounter($id_pelayanan, $id_history)
    {
        $this->load->library('uuid');
        $db_erm = $this->db->get_where('form_assesmen_awal_rajal', ['id_pelayanan' => $id_pelayanan])->row();
        if (!empty($db_erm)) {
