...
class Pelayanan_transport extends CI_Controller
{
	...
	public function getListTransport()
	{
		...
		// 	$data = $this->db->get_where('list_tindakan_apelkes', ['tipe_kamar' => 'TRANSPORTASI', 'status' => 'LAMA'])->result();
		// } else {
	...
	public function insert()
	{
		...
		$db_pel = $this->db->get_where("pelayanan", ['id_pelayanan' => $id_pelayanan])->row();
		$data = array(
			'id_tindakan' => uniqid(),
			'harga' => $harga,
			'frek' => $frek,
			...
			// 'id_dokter' => $id_dokter,
			'status_pembayaran' => ($db_pel->cara_bayar == '42')?'ditanggung':$this->input->post('status_pembayaran'),
		);

		$this->M_Poli->insert_tindakan($data, 'tindakan_pelayanan_tambahan');
		$out['status'] = "success";
	...
	{
		$id_tindakan_radiologi = $this->input->post('id_tindakan_apelkes');
		$this->M_Poli->delete_tindakan($id_tindakan_radiologi, 'tindakan_pelayanan_tambahan', 'id_tindakan');
		$out['status'] = "success";
		echo json_encode($out);
	}
	public function tampil_list_tindakan()
	{
		...
		list_tindakan_apelkes l, tindakan_pelayanan_tambahan t,staff s
		...
		for ($i = 0; $i < count($page_data); $i++) {
			...
			$tombol =   "<button class='btn btn-danger btn-icon-anim btn-square' data-toggle='modal' onclick='hapus_transport(\"" . $page_data[$i]->id_tindakan . "\",\"" . $id_pelayanan . "\",\"" . $page_data[$i]->nama .  "\")' '><i class='fa fa-trash '></i></button>";
			...
			$out[$i] = array($no, $tombol, $nama_tindakan, $harga, $frek, $total, $tanggal_pel, $nama_staff, $status);
		}
		if ($out == null) {
			echo '{"data":""}';
			exit;
	...
	public function tampil_total_harga()
	{
		...
		list_tindakan_apelkes l, tindakan_pelayanan_tambahan t
