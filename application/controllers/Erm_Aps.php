...
class Erm_Aps extends CI_Controller{
	...
	public function form($id_pelayanan, $id_history){
		...
		$page_data['page_content'] = 'erm_form/view_Erm_Eps';
...
public function form_riwayat($id_pelayanan, $id_history)
	...
	public function tampil_data(){
		...
		for($i = 0;$i < count($sql);$i++){
			$no = $i + 1;
			$tombol = "<button class='btn btn-success btn-icon-anim btn square'  onclick='pilih(\"" . $sql[$i]->id_uAps . "\")' '><i class='icon-rocket'></i></button>";
			$hapus = "<button class='btn btn-danger btn-icon-anim btn square' id='myHapus_".$sql[$i]->id_uAps."' onclick='hapus(\"" . $sql[$i]->id_uAps . "\")' '><i class='icon-trash'></i></button>";
