...
class Erm_ranap_persetujuan_kedokteran extends CI_Controller
{
    ...
    public function formpersetujuan($id_pelayanan, $id_history)
	{
		...
		$page_data['page_content'] = 'erm_form/Ranap/view_persetujuan_tindakan_dokter';
	...
	public function tampil_list_per_tindakan_dokter()
	{
		...
		$page_data = $this->M_Erm_ranap->selectPerTindakanDok($id_pelayanan);
	...
	public function getPerTindakanDok()
	{
		...
		$db = $this->db->get_where('form_ranap_persetujuan_tindakan_dokter', ['id_form_persetujuan_tindakan_dokter' => $id])->row_array();
		if (count($db) > 0) {
			$db = $db;
			$db['status_dt'] = 'found';
		} else {
	...
	public function insert_persetujuan_tindakan()
	{
		...
		$this->form_validation->set_rules('tgl_tindakan', 'Asal Rujuk');
		if ($this->form_validation->run()) {
			...
			// 	'no_rm' => $this->input->post('no_rm'),
			// );
			$this->M_Erm_ranap->insert($data, 'form_ranap_persetujuan_tindakan_dokter');
