...
<?php
define('WS_BHCE', 'on');
define('WS_BHCE_API', "https://api-test.tb-bche.id/v1/");
define('WS_BHCE_ORG_ID', "1971043");
define('WS_BHCE_AUTH_API', "https://api-test.tb-bche.id/v1/user/login");
...
function generateHeaderBHCE($params = array())
{
    $header_with_token = array('Content-Type' => 'application/json');

    if (defined('WS_BHCE') && WS_BHCE != 'off') {
        if (defined('WS_BHCE_AUTH_API')) {
            $urlWS = WS_BHCE_AUTH_API;
            $email = "1971043@bche.id";
            $pass = "12345678";
            $data = array(
                'email' => $email,
                'password' => $pass,
                ...
                if ($isUp) {
                    ...
                    if (!empty($result)) {
                        ...
                        if (isset($response->accessToken)) {
                            ...
                            $header_with_token['Authorization'] =  $response->accessToken;
                        }
                    }
                }
            } catch (Exception $e) {
...
function bhcePost($params, $kategori, $tipe)
{
    $ret = array(
        'status' => 0,
        'msg' => '',
    ...
    if (defined('WS_BHCE_API')) {
        ...
        try {
            ...
            if ($isUp) {
                ...
                // Set The Response Format to Json
                curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json', 'x-access-token: ' . $header['Authorization']));
...
function bhceGet($kategori, $tipe, $offset, $limit)
{
    $ret = array(
        'status' => 0,
        'msg' => '',
