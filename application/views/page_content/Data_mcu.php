<div class="panel panel-default card-view">
	<div class="panel-heading">
		<div class="pull-left">
			<h6 class="panel-title txt-dark"><span class="label label-success font-weight-100">DATA PASIEN MCU</span></h6>
		</div>
	...
	<div class="modal fade bs-example-modal-md" id="modal_edit_mcu_shap" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
		<div class="modal-dialog modal-md">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					<h5 class="modal-title" id="myLargeModalLabel"><i class="icon-user mr-10"></i> EDIT TINDAKAN MCU</h5>
				</div>
					...
					<div class="form-wrap">
							...
							<div class="row">
								<div class="col-md-10">
									<div class="form-group">
										<label class="control-label col-md-4 pt-5">Perusahaan / Jenis Penjamin</label>
										<!-- <div class="col-md-9 has-success">
											<input type="text" class="form-control " autocomplete="off" placeholder="NAMA TINDAKAN" id="upPerusahaan">
											<span class="help-block"></span>
										</div> -->
										<div class="col-md-7 has-success">
											<select class="form-control filled-input select2" placeholder="Choose a Category" tabindex="1" id="upPerusahaan">
												<?php
												$cara_bayar = $this->db->get('cara_bayar')->result_array();
												foreach ($cara_bayar as $row) :
												?>
													<option value="<?= $row["id_cara_bayar"] . '|' . $row["nama"]; ?>">
														<?= $row["nama"]; ?></option>
												<?php endforeach; ?>
											</select>
											<span class="help-block"></span>
						...
						<div class="row">
							<div class="col-md-2">
								...
								<button type="submit" class="btn btn-success mr-10" onclick="updateTindakanMcu()">SIMPAN</button>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="form-wrap">
						<!-- /formbody -->
						<!-- <div class="collapse" id="collap_edit_mcu">
							<div class="form-body">
								...
								<div class="col-md-6">
									<div class="form-group ">
										...
										<div class="col-md-9 has-success">
											<input type="text" class="form-control " id="upTindakan">
											<span class="help-block"></span>
										</div>
									</div>
								</div>
								...
								<div class="col-md-6">
									<div class="form-group ">
										...
										<div class="col-md-9 has-success">
											<input type="number" class="form-control " id="upBiaya" >
											<span class="help-block"></span>
										</div>
									</div>
								</div>
									...
									<div class="col-md-6">
										<div class="row">
											<div class="col-md-offset-3 col-md-9">
												<div type="submit" class="btn btn-success mr-10" onclick="up_tindakan_mcu()">SIMPAN</div>
...
<script>
	...
	// Amirul
	function edit_mcu_shap(id_mcu) {
		$.ajax({
			url: "<?php echo base_url() ?>Mcu/getDataMcu",
			data: {
				id_mcu: id_mcu,
			},
			type: 'POST',
			...
			success: function(data) {
				if (data.status_dt == "found") {
					$("#upId").val(data.id_mcu);
					$("#upPerusahaan").val(data.cara_bayar+'|'+data.perusahaan).change();
					// $("#upPerusahaan").val(data.cara_bayar);
					$("#modal_edit_mcu_shap ").modal('show');
				} else {
	...
	function updateTindakanMcu() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url() ?>Mcu/edit_tindakan_mcu_shap",
			dataType: 'json',
			data: {
				id: id,
			...
			success: function(data) {
				if (data.status == "success") {
					...
					$("#modal_edit_mcu_shap").modal('hide');
					$('#datable').DataTable().ajax.reload();
				} else {
					swal({
						title: "Gagal!",
...
<script>
	...
	function verif_mcu(id_mcu) {
		$.ajax({
			url: "<?= base_url() . 'Mcu/verif_mcu' ?>",
			data: {
				id_mcu: id_mcu,
			},
			type: 'POST',
