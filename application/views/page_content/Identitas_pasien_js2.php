...
<script>
	...
	// 	no_rm = $('#no_rm').val();
	// 	$.ajax({
	// 		url: "<?php echo base_url(); ?>Identitas_pasien/getDataPasien",
	// 		method: "POST",
	// 		data: {
	// 			pasien: no_rm
	// 		},
			...
			if (kelas != '') {
				$.ajax({
					...
					success: function(data) {
						...
						for (i = 0; i < data.length; i++) {
							html += '<option value=' + data[i].id_ruangan + '|' + data[i].id_bpjs + '>' + data[i].tipe + '</option>';
						}
...
<script type="text/javascript">
	function cek_peserta() {
		...
		$.ajax({
			url: "<?php echo base_url(); ?>Vclaim_bpjs/cek_peserta",
			method: "POST",
			data: {
				no_kartu: no_kartu,
				nik: nik,
			...
			success: function(response) {
				$('#modal_vclaim').modal('show');
				$('#peserta_bpjs').collapse('show');
				$('#bpjs_nama').val(response.nama);
				$('#bpjs_nik').val(response.nik);
				$('#jenis_kelamin').val(response.jk);
				$('#no_kartu').val(response.nokartu);
				$('#hakKelas').val(response.hakKelas);
				$('#jenisPeserta').val(response.jenisPeserta);
