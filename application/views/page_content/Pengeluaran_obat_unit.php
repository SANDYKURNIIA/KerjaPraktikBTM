...
<div class="panel panel-default card-view mt-20 ">
    <div class="panel-heading">
        <div class="pull-left">
            <h6 class="panel-title txt-dark"><span class="label label-success font-weight-100">PENGELUARAN OBAT</span></h6>
        </div>
        <div align="right">
            <div class="btn btn-primary btn-anim btn-sm " onclick="obatBebas()" style="margin-right: 40px;"><i class="icon-rocket"></i><span class="btn-text">PENGELUARAN OBAT</span>
                <div></div>
            </div>
        </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group ">
                                        ...
                                        <div class="col-md-9 has-success">
                                            <input type="date" class="form-control rounded-input" autocomplete="off" placeholder="NAMA" id="inNama">
...
<script type="text/javascript">
    function edit_data(id_pelayanan) {
        $.ajax({
            url: "<?= base_url() . 'Pengeluaran_obat_unit/getDataPasien' ?>",
            data: {
    ...
    function getObatReturn(id_resep) {
        $.ajax({
            url: "<?php echo base_url(); ?>Pengeluaran_obat_unit/getNamaObatReturn",
            method: "POST",
            data: {
...
<script type="text/javascript">
    ...
    function insert() {
        ...
        keterangan = $('#inKet').val();
            ...
            $.ajax({
                url: "<?= base_url() . 'Pengeluaran_obat_unit/insert_obat_bebas' ?>",
                method: "POST",
                dataType: 'json',
                data: {
                    nama: nama,
    ...
    function insert_Obat() {
        ...
        $.ajax({
            url: "<?= base_url() . 'Pengeluaran_obat_unit/insert_tindakan_obat_bebas' ?>",
            method: "POST",
            dataType: 'json',
            cache: false,
            data: {
    ...
    function hapus_obat(id, nama) {
        swal({
            ...
            $().ready(function() {
                $.ajax({
                    url: "<?php echo base_url() ?>Pengeluaran_obat_unit/hapus_obat",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        id: id,
    ...
    function hapus_tindakan(id, nama) {
        swal({
            ...
            $().ready(function() {
                $.ajax({
                    url: "<?php echo base_url() ?>Pengeluaran_obat_unit/hapus_tindakan",
                    method: "POST",
                    dataType: 'json',
                    data: {
                        id: id,
    ...
    function reload_data_obat(id) {
        ...
        $('#tableobat').DataTable({
            ...
            "ajax": {
                "url": '<?php echo base_url('Pengeluaran_obat_unit/tampil_tindakan_obat_bebas'); ?>',
                "type": 'POST',
                "data": {
                    id: id,
                },
    ...
    $(document).ready(function() {
        $('#datable').DataTable({
            ...
            "ajax": '<?php echo base_url('Pengeluaran_obat_unit/Tampil_obat_bebas'); ?>',
            "deferRender": true,
            "processing": true,
            "order": [],
            "columnDefs": [{
    ...
    function tampilRangePermit(mulai, akhir) {
        ...
        $('#datable').DataTable({
            ...
            "ajax": {
                "url": '<?= base_url('Pengeluaran_obat_unit/Tampil_obat_bebas'); ?>',
                "type": 'POST',
                "data": {
                    mulai: mulai,
                    akhir: akhir
