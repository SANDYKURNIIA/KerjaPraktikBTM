...
<div class="panel panel-default card-view mt-20 ">
	<div class="panel-heading">
		<div class="pull-left">
			<h6 class="panel-title txt-dark"><span class="label label-success font-weight-100">RIWAYAT PASIEN RAWAT
					INAP</span></h6>
		</div>
							...
							<div class="row">
								<div class="col-md-6">
									<div class="col-md-9 col-sm-12 col-xs-12">
										<button type="submit" class="btn btn-success btn-square" id="btnYakin">YAKIN</button>
									</div>
								</div>
								<div class="col-md-6">
									<div class="">
...
<script type="text/javascript">
	function check_out_yang_asli(id_history, nama) {
		swal({
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#datable').DataTable({
			...
			"ajax": '<?php echo base_url('Rawatinap/tampil_data_ranap_riwayat'); ?>',
			"deferRender": true,
			"processing": true,
			"order": [],
			"columnDefs": [{
	...
	function tampilRangePermit(mulai, akhir) {
		...
		$('#datable').DataTable({
			...
			"ajax": {
				"url": '<?= base_url('Rawatinap/tampil_data_ranap_riwayat'); ?>',
				"type": 'POST',
				"data": {
					mulai: mulai,
					akhir: akhir
...
<script>
	function kembalikankeruangan(id_history, nama) {
		swal({
			title: "Apakah kamu yakin?",
			text: "Mengembalikan Pasien " + nama + " Ke Rawat Inap?",
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#3cb878",
			confirmButtonText: "Yakin",
			...
			$().ready(function() {
				$.ajax({
					url: "<?php echo base_url() ?>Rawatinap/kembalikankeruangan",
					method: "POST",
					dataType: 'json',
					data: {
						id_history: id_history,
					...
					success: function(data) {
						if (data.status == "success") {
							swal({
								title: "good job!",
								type: "success",
								text: "Pasien Berhasil Dikembalikan keruangan",
								confirmButtonColor: "#3cb878",
							});
							$('#datable').DataTable().ajax.reload();
						} else {
	...
	function kembalikankeruangan_input(id_history, nama) {
		swal({
			title: "Kembalikan Pasien?",
			text: "Pasien atas nama " + nama + " Ke Rawat Inap? Silahkan, hubungi admin ruangan. Terimakasih",
			type: "warning",
			confirmButtonColor: "#3cb878",
			closeOnConfirm: true
		});
