					...
					<div class="col-sm-13">
						<div class="panel panel-default card-view pb-0">
                            ...
                            <input type="text" class="form-control rounded-input text-center" value="<?= $count_data['jumlah']; ?>" disabled>
								...
								<input type="hidden" id="no_antrian" value="<?php $in = 'p'; $no= $antrian_data['no_antri']; echo ($in.$no); ?>" disabled>
...
<script type="text/javascript">
    $(document).ready(function() {
		...
		if (no_antrian == 'p0') {
            ...
            $('#tabledata').DataTable({
                    ...
                    "ajax": '<?php echo base_url('Poli_internis/tampilAntrian'); ?>',
                    "deferRender": true,
                    "processing": true,
                    "order": [],
                    "columnDefs": [{
	...
	function skip_data(id_antrian) {
			$.ajax({  
				url : "<?= base_url() ?>Poli_internis/updateskip",
				method: 'POST',
				dataType: 'json',
				data : {
        ...
        function nextAntrian() {
			...
			$.ajax({  
				url : "<?= base_url() ?>Poli_internis/updatenext",
				method: "POST",
				dataType: 'json',
				data : {
					id_antrian: id_antrian, 
        ...
        function playSuara(no_antri,nama) {
			$nomor = "p" +no_antri;
				$.ajax({  
					url : "<?= base_url() ?>Poli_internis/playSuara",					
					method: "POST",
					dataType: 'json',
					data : {
						nomor: $nomor, 
