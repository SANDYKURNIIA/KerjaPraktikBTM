...
<div class="panel panel-default card-view mt-20 ">
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<div class="col-md-9 has-success" id="the-basics">
											<select class="form-control filled-input select2" placeholder="Choose a Category" tabindex="1" id="tipe_masuk" name="inTipeMasuk" disabled>
	...
	<script type="text/javascript">
		function edit_data_kunjungan(id_pelayanan, id_history) {
			...
			$.ajax({
				url: "<?= base_url() . 'Pasien/getddata_rajal' ?>",
				data: {
				...
				success: function(data) {
					if (data.status_dt == "found") {
						...
						if (data.jenis_pelayanan == 'UGD') {
							$("#tipe_masuk").val('1').change();
						} else if (data.jenis_pelayanan == 'POLI') {
							$("#tipe_masuk").val('2').change();
						} else {
							$("#tipe_masuk").val('4').change();
						}
						...
						// // $('#inNaPol  option[value="'+data.nama_poli+'"]').prop("selected", true);
						// // $('#inNaPol option[value='+data.nama_poli+']').attr('selected','selected');
						// $('select[id="inNaPol"]').val(data.nama_poli);
						$("#inNaPol").val(data.nama_poli).change();
						getDokterPoli1(data.nama_poli,data.dpjp);
						$("#inDPJP").val(data.dpjp).change();
						$("#inDPJP1").val(data.dpjp).change();

						// console.log(data.dpjp);
		...
		function getDokterUGD() {
			var tipe_masuk = $('#tipe_masuk').val();
			if (tipe_masuk == '1') {
				$.ajax({
		...
		function getDokterPoli1(poli, dpjp) {
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			// 				if (jenis_pasien == 'baru' && (poli != '146582' && poli != '15487956')) {
			// 					$('#inBiayaAdm').val(10000);
			// 					var c = 10000;
			// 				} else {
			// 					$('#inBiayaAdm').val(0);
			...
			// 				$("#inTotal").val(total);
			// 			} else { //asuransi
			// 				if ((cara_bayar == '333' || cara_bayar == '31' || cara_bayar == '15') && tipe == '4') { //timah,inhealth,bri life prioritas
			...
			// 						var c = 0;
			// 					}
			// 					$("#inBiayaRS").val(data.rs_timah_pagi);
			// 					$('#inBiayaDok').val(data.jasmed_timah_pagi);
			// 					var a = parseInt(data.rs_timah_pagi);
			// 					var b = parseInt(data.jasmed_timah_pagi);

			// 					var total = Number(a) + Number(b) + Number(c);
			// 					$("#inTotal").val(total);

			// 				} else if ((cara_bayar == '32' || cara_bayar == '204') && tipe == '4') { //PLN prioritas
			// 					if (jenis_pasien == 'baru') {
			// 						$('#inBiayaAdm').val(10000);
			// 						var c = 10000;
			// 					} else {
	...
	<script type="text/javascript">
		function edit_rawat_jalan() {
			...
			dpjp = $('#inDPJP').val();
			dpjp = (dpjp == '' || dpjp == '-') ? $('#inDPJP1').val() : dpjp;
			NaPol = $('#inNaPol').val();
	...
	<script type="text/javascript">
		$(document).ready(function() {
			$('#datable').DataTable({
				...
				"ajax": '<?php echo base_url('Pasien/tampil_datarajal'); ?>',
				"deferRender": true,
				"processing": true,
				"order": [],
				"columnDefs": [{
		...
		function tampilHariIni() {
			...
			$('#datable').DataTable({
				...
				"ajax": {
					"url": '<?= base_url('Pasien/tampil_datarajal'); ?>',
					"type": 'POST',
