...
<script type="text/javascript">
	...
	function tindakan_fisio(id_pelayanan, id_history, jenis_pelayanan) {
		$.ajax({
			...
			success: function(data) {
				...
				for (i = 0; i < data.fisio.length; i++) {
					// console.log(data.fisio[0].nama);
					html += '<option value=' + data.fisio[i].id_list_tindakan + '|' + data.fisio[i].harga_sarana + '|' + data.fisio[i].harga_jasa + '|' + data.fisio[i].nama + '>' + data.fisio[i].nama + '</option>';
				}
	...
	function check_out(id_pelayanan, id_history) {
		swal({
			title: "Warning?",
			text: "Apakah kamu yakin ingin check out pasien ?",
			type: "warning",
			showCancelButton: true,
			confirmButtonColor: "#3cb878",
			confirmButtonText: "Yakin",
...
<script type="text/javascript">
	...
	function reload_table(id_pelayanan) {
		...
		$('#datable').DataTable({
			...
			"ajax": {
				"url": '<?php echo base_url('Pasien/tampil_data_PasienPolifisio'); ?>',
				"type": 'POST',
				"data": {
					id_pelayanan: id_pelayanan
				},
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#datable').DataTable({
			...
			"ajax": '<?php echo base_url('Pasien/tampil_data_PasienPolifisio'); ?>',
			"deferRender": true,
			"processing": true,
			"order": [],
			"columnDefs": [{
...
<script type="text/javascript">
	...
	function insert_fisio() {
		...
		dataString = 'id=' + ID +
			...
			'&nama_dokter=' + nama_dokter + '&nama_tindakan=' + nama_tindakan;
		$.ajax({
