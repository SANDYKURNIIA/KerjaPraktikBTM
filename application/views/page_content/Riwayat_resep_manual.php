...
<div class="panel panel-default card-view mt-20 ">
	<div class="panel-heading">
		<div class="pull-left">
			<h6 class="panel-title txt-dark"><span class="label label-success font-weight-100">RIWAYAT PASIEN FARMASI</span></h6>
		</div>
						...
						<div class="form-wrap">
							...
							<div class="form-actions mt-10">
								<div class="row ">
									...
									<hr width="95%">
									<input type="hidden" id="inPelObat">
									<input type="hidden" id="inHisObat">
...
<script type="text/javascript">
	...
	function tampilTindakanFarmasi(idPel, idHis) {
		$('#inPelObat').val(idPel);
		$('#inHisObat').val(idHis);
	...
	function cetak_retur() {
		id_history = $('#inHisResep').val();
		id = $('#inPelResep').val();
		window.open('<?= base_url('Apotik_poli/') ?>print_retur_obat_bebas/' + id + '/' + id_history);
	}
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#datable').DataTable({
			...
			"ajax": '<?php echo base_url('Apotik_poli/Tampil_Riwayat_resep_manual'); ?>',
			"deferRender": true,
			"processing": true,
			"order": [],
			"columnDefs": [{
	...
	function tampilRangePermit(mulai, akhir) {
		...
		$('#datable').DataTable({
			...
			"ajax": {
				"url": '<?= base_url('Apotik_poli/Tampil_Riwayat_resep_manual'); ?>',
				"type": 'POST',
				"data": {
					mulai: mulai,
					akhir: akhir
