...
<div class="panel panel-default card-view">
    <div class="panel-heading">
        <div class="pull-left">
            <h6 class="panel-title txt-dark"><span class="span span-success">TINDAKAN RADIOLOGI MCU</span></h6>
        </div>
...
<div class="panel-wrapper collapse in">
    ...
    <div class="modal fade bs-example-modal-lg" id="modal_edit_radio_mcu" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true" style="display: none;">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header">
...
<script type="text/javascript">
    function up_tindakan_mcu() {
		...
		$.ajax({
			url: "<?= base_url() . 'mcu/insert_tindakan_radio_mcu' ?>",
			method: "POST",
			dataType: 'json',
			cache: true,
			data: {
    ...
    function edit_radio_mcu(id_daftar_tindakan){
            $.ajax({
                url: "<?php echo base_url() ?>mcu/getDataRadioMcu",
                data: {
                    id_daftar_tindakan: id_daftar_tindakan,
                },
                type: 'POST',
                dataType: 'json',
                success: function(data) {
                    if (data.status_dt == "found") {
                        $("#upId").val(data.id_daftar_tindakan);
                        $("#inTindakan").val(data.nama);
                        $("#inBiaya").val(data.harga);
                        $("#inCost").val(data.harga_cost);
                        $("#inStatus").val(data.status);
                        $("#modal_edit_radio_mcu").modal('show');
                    } else {
        ...
        function updateTindakan(){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url() ?>mcu/edit_radio_mcu",
                dataType: 'json',
                data: {
                    id: id,
                    nama: nama,
                ...
                success: function(data) {
                    if (data.status == "success") {
                        ...
                        $("#modal_edit_radio_mcu").modal('hide');
                        $('#datable').DataTable().ajax.reload();
                    } else {
                        swal({
                            title: "Gagal!",
    ...
    $(document).ready(function () {
                ...
                $('#datable').DataTable({
                        ...
                        "sLast":     "Terakhir"
                            ...
                            "ajax": '<?php echo base_url('mcu/tampil_tindakan_radio_mcu'); ?>',    
                            "deferRender": true,
                            "processing": true,
                            "order": [], 
                            "columnDefs": [
