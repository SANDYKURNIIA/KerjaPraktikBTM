					...
					<div class="col-sm-13">
						<div class="panel panel-default card-view pb-0">
                            ...
                            <input type="text" class="form-control rounded-input text-center" value="<?= $count_data['jumlah']; ?>" disabled>
								...
								<input type="hidden" id="no_antrian" value="<?php $in = 'f'; $no= $antrian_data['no_antri']; echo ($in.$no); ?>" disabled>
...
<script type="text/javascript">
    $(document).ready(function() {
		...
		if (no_antrian == 'f0') {
            ...
            $('#tabledata').DataTable({
                    ...
                    "ajax": '<?php echo base_url('Poli_obgyne/tampilAntrian'); ?>',
                    "deferRender": true,
                    "processing": true,
                    "order": [],
                    "columnDefs": [{
	...
	function skip_data(id_antrian) {
			$.ajax({  
				url : "<?= base_url() ?>Poli_obgyne/updateskip",
				method: 'POST',
				dataType: 'json',
				data : {
        ...
        function nextAntrian() {
			...
			$.ajax({  
				url : "<?= base_url() ?>Poli_obgyne/updatenext",
				method: "POST",
				dataType: 'json',
				data : {
					id_antrian: id_antrian, 
        ...
        function playSuara(no_antri,nama) {
			$nomor = "o" +no_antri;
				$.ajax({  
					url : "<?= base_url() ?>Poli_obgyne/playSuara",					
					method: "POST",
					dataType: 'json',
					data : {
						nomor: $nomor, 
