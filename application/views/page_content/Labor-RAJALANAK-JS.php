...
function reload_data_labor_ANAK(id_pelayanan) {  
    ...
    $('#tablelaborANAK').DataTable({
        ...
        "ajax": {
            "url": '<?php echo base_url('Labor/tampil_all_labor_anak'); ?>',
            "type": 'POST',
            "data": {
...
<script type="text/javascript">
	function detail_tindakan_labor_anak(id_tindakan_labor) {
		$.ajax({
			...
			success: function(data) {
				if (data.status_dt == "found") {
					$('#detailTindakanLaborANAK').collapse('toggle');
					$("#outNamaANAK").val(data.nama);
					$("#outFrekANAK").val(data.frek);
					$("#outTanggalANAK").val(data.tanggal_req);
					$("#outHargaANAK").val(data.harga);
					$("#outRingANAK").val(data.ringkasan);
					$("#outKetaANAK").val(data.keterangan);
				} else {
					swal({
						title: "Gagal!",
						type: "warning",
...
<!-- END ANAK -->
	 ...
	 <script type="text/javascript">
	 	 function insert_anak_darah() {
			...
			inJenisPasienANAK=$('#inMasukANAK').val();
			swal({   
	 ...
	 function insert_anak_guldarah() {
		...
		function insert_gol_darah_anak() {
