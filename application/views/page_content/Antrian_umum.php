					...
					<div class="col-sm-13">
						<div class="panel panel-default card-view pb-0">
                            ...
                            <input type="text" class="form-control rounded-input text-center" value="<?= $count_data['jumlah']; ?>" disabled>
								...
								<input type="hidden" id="no_antrian" value="<?php $in = 'l'; $no= $antrian_data['no_antri']; echo ($in.$no); ?>" disabled>
...
<script type="text/javascript">
    $(document).ready(function() {
		...
		if (no_antrian == 'l0') {
            ...
            $('#tabledata').DataTable({
                    ...
                    "ajax": '<?php echo base_url('Poli_umum/tampilAntrian'); ?>',
                    "deferRender": true,
                    "processing": true,
                    "order": [],
                    "columnDefs": [{
	...
	function skip_data(id_antrian) {
			$.ajax({  
				url : "<?= base_url() ?>Poli_umum/updateskip",
				method: 'POST',
				dataType: 'json',
				data : {
        ...
        function nextAntrian() {
			...
			$.ajax({  
				url : "<?= base_url() ?>Poli_umum/updatenext",
				method: "POST",
				dataType: 'json',
				data : {
					id_antrian: id_antrian, 
        ...
        function playSuara(no_antri,nama) {
			$nomor = "u" +no_antri;
				$.ajax({  
					url : "<?= base_url() ?>Poli_umum/playSuara",					
					method: "POST",
					dataType: 'json',
					data : {
						nomor: $nomor, 
