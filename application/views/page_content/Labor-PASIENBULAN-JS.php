<script type="text/javascript">
					function reload_data_labor_BULAN(id_pelayanan) {    
                        ...
                        $('#tablelaborBULAN').DataTable({
                            ...
                            "ajax": {
                                "url": '<?php echo base_url('Labor/tampil_all_labor_bulan_sendiri'); ?>',
                                "type": 'POST',
                                "data": {
...
<script type="text/javascript">
				...
				// Labor
				function insert_labor_bulan() { 
                    a = $("#inTindakanLabor_BULAN").val();
                    splitDiag = a.split("|");
                    harga = parseFloat(splitDiag[1]);
                    frek = parseFloat($("#inJumlahLabor_BULAN").val());
                    total = harga * frek;
                    id_pel_lab = $('#id_pel_lab_BULAN').val();
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.status == "success") {
								...
								$('#outBiayaTindakanLabor_BULAN').val('');
								$('#inJumlahLabor_BULAN').val('');
								$('#outTotalLabor_BULAN').val('');
								$('#tablelaborBULAN').DataTable().ajax.reload();
                                $('#outTotalHargaBULAN').DataTable().ajax.reload(); 
                            } else {
                                swal({
                                    title: "Gagal!",
                                    type: "warning",
				...
				function pilihTindakanLabor_BULAN() {
                    a = $("#inTindakanLabor_BULAN").val();
                    splitDiag = a.split("|");

                    harga = parseFloat(splitDiag[1]);
                    $("#outBiayaTindakanLabor_BULAN").val(convertToRupiah(harga));
                    document.getElementById("inJumlahLabor_BULAN").value = "1";
                    document.getElementById("outTotalLabor_BULAN").value = convertToRupiah(harga);
                }
			...
			function aksi_labor(id_pelayanan, id_history) {
				$.ajax({
					...
					success: function (data) {
						if (data.status_dt == "found") {
							$("#id_pel_lab_BULAN").val(data.id_pelayanan);
