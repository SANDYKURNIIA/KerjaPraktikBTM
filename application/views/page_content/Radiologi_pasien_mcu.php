							...
							<div class="row">
								...
								<div class="col-md-6" style="margin-top:-1em;">
									<div class="form-group pull-right" style="margin-right:20px;">
										<button id="btn_upload" type="submit" class="btn btn-success btn-anim  btn-sm ml-20 mt-5"><i class="icon-rocket"></i><span class="btn-text">SIMPANX</span>
									</div>
								</div>
							</div>
									...
									<div class="panel-body" style="margin-left:-1em;">
										...
										<div class="col-md-6">
											<div class="form-group ">
												...
												<div class="col-md-9">
													<input type="text" class="form-control" disabled id="ruang" value="MCU">
													<span class="help-block"></span>
												</div>
											</div>
										</div>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('#formdata').submit(function(e) {
			...
			$.ajax({
				url: '<?php echo base_url(); ?>Radiologi/post_radiologi_mcu',
				type: "POST",
				data: formData,
				processData: false,
				contentType: false,
		...
		$('#datable').DataTable({
			...
			"ajax": '<?php echo base_url('Radiologi/tampil_pasien_mcu'); ?>',
			"deferRender": true,
			"processing": true,
			"order": [],
			"columnDefs": [{
	...
	function pindah_data(id_pelayanan, id_tindakan_radiologi) {
		...
		$.ajax({
			...
			dataType: 'json',
			data: JSON.stringify(postData), // Mengubah objek data menjadi string JSON
			success: function(data) {
				if (data.status == "success") {
					...
					reload_data_radiologi(data.id_pelayanan); // Memastikan variabel dikirim dengan benar
					reload_total_radiologi(data.id_pelayanan); // Memastikan variabel dikirim dengan benar
					swal({
						title: "Berhasil!",
						type: "success",
	...
	function reload_data_radiologi(id_mcu) {
		...
		$('#tableradiologi').DataTable({
			...
			"ajax": {
				"url": '<?php echo base_url('Radiologi/tampil_mcu_radiologi'); ?>',
				"type": 'POST',
				"data": {
					id_pelayanan: id_mcu
				},
	...
	function reload_total_radiologi(id_pelayanan) {
		...
		$('#outTotalHargaRadiologi').DataTable({
			...
			"ajax": {
				"url": '<?php echo base_url('Radiologi/tampil_total_radiologi_mcu'); ?>',
				"type": 'POST',
				"data": {
					id_pelayanan: id_pelayanan
				},
	...
	function edit_data_tindakan(id_mcu) {
		$.ajax({
			url: "<?= base_url() . 'Radiologi/getdata_radiologiMcuALL' ?>",
			data: {
				pelayanan: id_mcu,
				//history: id_history
			},
			...
			success: function(data) {
				if (data.status_dt == "found") {
					$("#idPelayanan").val(data.id_mcu);
					$("#id_pel_rad").val(data.id_mcu);
					$("#tgl_lahir").val(data.tgl_lahir);
					$("#no_rm").val(data.no_rm);
					$("#cara_bayar").val(data.cara_bayar);
					$("#inNamaPasien").val(data.nama_pasien);
	...
	function edit_tindakan_radiologi(id_mcu, id_tindakan_radiologi) {
		$.ajax({
			url: "<?= base_url() . 'Radiologi/getdatamcu_formById' ?>",
			data: {
				pelayanan: id_mcu,
	...
	function edit_gambar_radiologi(id_mcu, id_tindakan_radiologi) {
		$.ajax({
			url: "<?= base_url() . 'Radiologi/getdatamcu_formById' ?>",
			data: {
				pelayanan: id_mcu,
	...
	function aksi_radiologi(id_mcu, id_tindakan_radiologi) {
		$.ajax({
			url: "<?= base_url() . 'Radiologi/getdatamcu_formById' ?>",
			data: {
				pelayanan: id_mcu,
